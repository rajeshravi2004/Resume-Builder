...
export const Templates = () => {
  ...
  return <div className="design-layout">
    <section className="design-panel">
      ...
      {tab === 'templates' && <div className="template-picker"><div className="panel-intro"><h2>Curated templates</h2><p>All designs are ATS-conscious and fully customisable.</p></div><div className="template-grid">{TEMPLATE_PRESETS.map(preset => <button key={preset.id} className={`template-card ${design.template === preset.id ? 'active' : ''}`} onClick={() => applyTemplate(preset.id, preset.patch)}><div className="template-art" style={{ '--p': preset.patch.primaryColor, '--a': preset.patch.accentColor, '--side': preset.patch.layout === 'two-column' ? '31%' : '0%' }}><div className="template-art-head"><i /><b /></div><div className="template-art-body"><span /><span /><span className="short" /><em /><em /></div></div><div className="template-card-copy"><span><strong>{preset.name}</strong><small>{preset.category}</small></span>{design.template === preset.id && <Check size={16} />}</div><p>{preset.description}</p></button>)}</div></div>}
